...
function svgToPng(name) {
    return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    src = "./out/".concat(name, ".svg");
                    dest = "./out/".concat(name, ".png");
...
function createImage(idx) {
    ...
    if (face[takenFaces]) {
        ...
        writeFileSync("./out/".concat(idx, ".json"), JSON.stringify(meta));
        writeFileSync("./out/".concat(idx, ".svg"), final);
        svgToPng(idx);
    }
}
if (!existsSync('./out')) {
...
readdirSync('./out').forEach(function (f) { return rmSync("./out/".concat(f)); });
do {
    createImage(idx);
    idx--;
} while (idx >= 0);
